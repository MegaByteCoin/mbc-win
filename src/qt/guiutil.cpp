...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MBC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no mbc: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert mbc:// to mbc:
    //
    //    Cannot handle this later, because mbc:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open mbc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MBC.lnk";
}
...
{
    return GetAutostartDir() / "mbc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a mbc.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the mbc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add mbc app to startup item list
