...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call MBC with fAggressive=true to
    // get as much data as possible.
    // Rewrite mbcd data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> mbcdData;
    bool allOK = dbenv.MBC(newFilename, true, mbcdData);
    if (mbcdData.empty()) {
        LogPrintf("MBC(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("MBC(aggressive) found %u records\n", mbcdData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, mbcdData) {
        if (fOnlyKeys) {
